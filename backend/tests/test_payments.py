...
async def test_capture_adds_credits_on_completed():
    with (
        ...
        patch("payments.add_credits_and_record_transaction", return_value=10),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_capture_records_transaction():
    with (
        ...
        patch("payments._capture_order", new_callable=AsyncMock, return_value=True),
        patch("payments.add_credits_and_record_transaction", return_value=10) as mock_atomic,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(
    ...
    assert response.status_code == 200
    mock_atomic.assert_called_once_with(1, 3.00, 10, "ORDER-XYZ")
